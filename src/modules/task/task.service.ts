...
export class TaskService {
  ...
  async findByProject(project: string, user: string, { limit = 50, offset = 0 }): Promise<Task[]> {
    const tasks = await this.taskRepository.find({
  ...
  async findAllByProject(project: string, user: string, { limit = 50, offset = 0 }): Promise<Task[]> {
  ...
  async createTask(newTask: Partial<Task>, user: User): Promise<Task> {
    const project = await this.projectService.findOne(newTask.project, user.id);
  ...
  async updateTask(id: string, patch: Partial<Task>, user: string): Promise<Task> {
  ...
  async updateParent(id: string, parentId: string, user: string): Promise<Task> {
  ...
  async deleteTask(id: string, user: string): Promise<Task> {
    if (!id || id === '') {
      throw new HttpException('Task not found', HttpStatus.NOT_FOUND);
    }
    const task = await this.taskRepository.findOne({
